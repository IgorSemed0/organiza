...
export default function Trash({ items, filters }) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        router.get(route('admin.membro_workplaces.trash'), { search }, { preserveState: true });
    };

    const handleRestore = (id: number) => {
        if (confirm('Tem a certeza que deseja restaurar este membro de espaço de trabalho?')) {
            post(route('admin.membro_workplaces.restore', id), {
                onSuccess: () => {
                    reset();
    ...
    const handlePurge = (id: number) => {
        if (confirm('Tem a certeza que deseja eliminar permanentemente este membro de espaço de trabalho?')) {
            deleteRequest(route('admin.membro_workplaces.purge', id), {
                onSuccess: () => {
                    reset();
                    router.reload({ only: ['items', 'filters'] });
                },
    ...
    return (
        <AdminLayout title="Lixo de Membros de Espaços de Trabalho">
            <Head title="Lixo de Membros de Espaços de Trabalho" />
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <CardTitle>Lixo de Membros de Espaços de Trabalho</CardTitle>
                ...
                <CardContent>
                    ...
                    <form onSubmit={handleSearch} className="mb-4 flex space-x-2">
                        <div className="relative flex-1">
                            ...
                            <Input
                                type="text"
                                placeholder="Pesquisar membro de espaço de trabalho eliminado..."
                                value={search}
