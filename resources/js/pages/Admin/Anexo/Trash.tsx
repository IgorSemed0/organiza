...
export default function Trash({ items, filters }) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        router.get(route('admin.anexos.trash'), { search }, { preserveState: true });
    };

    const handleRestore = (id: number) => {
        if (confirm('Tem a certeza que deseja restaurar este anexo?')) {
            post(route('admin.anexos.restore', id), {
                onSuccess: () => {
                    reset();
    ...
    const handlePurge = (id: number) => {
        if (confirm('Tem a certeza que deseja eliminar permanentemente este anexo?')) {
            deleteRequest(route('admin.anexos.purge', id), {
                onSuccess: () => {
                    reset();
                    router.reload({ only: ['items', 'filters'] });
                },
    ...
    return (
        <AdminLayout title="Lixo de Anexos">
            <Head title="Lixo de Anexos" />
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <CardTitle>Lixo de Anexos</CardTitle>
                ...
                <CardContent>
                    ...
                    <form onSubmit={handleSearch} className="mb-4 flex space-x-2">
                        <div className="relative flex-1">
                            ...
                            <Input
                                type="text"
                                placeholder="Pesquisar anexo eliminado..."
                                value={search}
