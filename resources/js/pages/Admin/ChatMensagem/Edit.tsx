...
export default function Edit({ item, quadros, users }) {
    const { data, setData, put, processing, errors } = useForm({
        ...
        vc_texto_mensagem: item.vc_texto_mensagem || '',
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('admin.chat_mensagems.update', item.id));
    };

    return (
        <AdminLayout title="Editar Mensagem de Chat">
            <Head title="Editar Mensagem de Chat" />
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <CardTitle>Editar Mensagem de Chat</CardTitle>
