...
export default function Index({ items, filters }) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        router.get(route('admin.workplaces.index'), { search }, { preserveState: true });
    };

    const handleDelete = (id: number) => {
        if (confirm('Tem a certeza que deseja eliminar este espaço de trabalho?')) {
            router.delete(route('admin.workplaces.destroy', id));
        }
    };

    return (
        <AdminLayout title="Gestão de Espaços de Trabalho">
            <Head title="Gestão de Espaços de Trabalho" />
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <CardTitle>Gestão de Espaços de Trabalho</CardTitle>
                        <div className="flex space-x-2">
                            <Link href={route('admin.workplaces.trash')}>
                                <Button variant="outline">
                                    <Trash2 className="mr-2 h-4 w-4" />
                                    Lixo
                                </Button>
                            </Link>
                            <Link href={route('admin.workplaces.create')}>
                                <Button>
                                ...
                                {items.data.map((item) => (
                                    <tr key={item.id}>
                                        ...
                                        <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                                            <div className="flex space-x-2">
                                                <Link href={route('admin.workplaces.show', item.id)}>
                                                    <Button variant="outline" size="sm">Ver</Button>
                                                </Link>
                                                <Link href={route('admin.workplaces.edit', item.id)}>
