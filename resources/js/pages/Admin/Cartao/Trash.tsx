...
export default function Trash({ items, filters }) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        router.get(route('admin.cartaos.trash'), { search }, { preserveState: true });
    };

    const handleRestore = (id: number) => {
        if (confirm('Tem a certeza que deseja restaurar este cartão?')) {
            post(route('admin.cartaos.restore', id), {
                onSuccess: () => {
                    reset();
    ...
    const handlePurge = (id: number) => {
        if (confirm('Tem a certeza que deseja eliminar permanentemente este cartão?')) {
            deleteRequest(route('admin.cartaos.purge', id), {
                onSuccess: () => {
                    reset();
                    router.reload({ only: ['items', 'filters'] });
                },
    ...
    return (
        <AdminLayout title="Lixo de Cartões">
            <Head title="Lixo de Cartões" />
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <CardTitle>Lixo de Cartões</CardTitle>
                ...
                <CardContent>
                    ...
                    <form onSubmit={handleSearch} className="mb-4 flex space-x-2">
                        <div className="relative flex-1">
                            ...
                            <Input
                                type="text"
                                placeholder="Pesquisar cartão eliminado..."
                                value={search}
