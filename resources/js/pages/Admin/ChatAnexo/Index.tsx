...
export default function Index({ items, filters }) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        router.get(route('admin.chat_anexos.index'), { search }, { preserveState: true });
    };

    const handleDelete = (id: number) => {
        if (confirm('Tem a certeza que deseja eliminar este anexo de chat?')) {
            router.delete(route('admin.chat_anexos.destroy', id));
        }
    };

    return (
        <AdminLayout title="Gestão de Anexos de Chat">
            <Head title="Gestão de Anexos de Chat" />
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <CardTitle>Gestão de Anexos de Chat</CardTitle>
                        <div className="flex space-x-2">
                            <Link href={route('admin.chat_anexos.trash')}>
                                <Button variant="outline">
                                    <Trash2 className="mr-2 h-4 w-4" />
                                    Lixo
                                </Button>
                            </Link>
                            <Link href={route('admin.chat_anexos.create')}>
                                <Button>
                ...
                <CardContent>
                    <form onSubmit={handleSearch} className="mb-4 flex space-x-2">
                        <div className="relative flex-1">
                            ...
                            <Input
                                type="text"
                                placeholder="Pesquisar anexo de chat..."
                                value={search}
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead className="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-300">Mensagem de Chat</th>
                            ...
                            <tbody className="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">
                                {items.data.map((item) => (
                                    <tr key={item.id}>
                                        <td className="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">{item.chat_mensagem?.vc_texto_mensagem || 'N/A'}</td>
                                        ...
                                        <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                                            <div className="flex space-x-2">
                                                <Link href={route('admin.chat_anexos.show', item.id)}>
                                                    <Button variant="outline" size="sm">Ver</Button>
                                                </Link>
                                                <Link href={route('admin.chat_anexos.edit', item.id)}>
