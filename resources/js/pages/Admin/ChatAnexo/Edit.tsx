...
export default function Edit({ item, chatMensagens, users }) {
    const { data, setData, put, processing, errors } = useForm({
        it_id_chat_mensagem: item.it_id_chat_mensagem?.toString() || '',
        it_id_user_upload: item.it_id_user_upload?.toString() || '',
        vc_nome_arquivo: item.vc_nome_arquivo || '',
        vc_caminho_arquivo: item.vc_caminho_arquivo || '',
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('admin.chat_anexos.update', item.id));
    };

    return (
        <AdminLayout title="Editar Anexo de Chat">
            <Head title="Editar Anexo de Chat" />
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <CardTitle>Editar Anexo de Chat</CardTitle>
