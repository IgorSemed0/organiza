...
export default function Index({ items, filters }) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        router.get(route('admin.membro_quadros.index'), { search }, { preserveState: true });
    };

    const handleDelete = (id: number) => {
        if (confirm('Tem a certeza que deseja eliminar este membro de quadro?')) {
            router.delete(route('admin.membro_quadros.destroy', id));
        }
    };

    return (
        <AdminLayout title="Gestão de Membros de Quadros">
            <Head title="Gestão de Membros de Quadros" />
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <CardTitle>Gestão de Membros de Quadros</CardTitle>
                        <div className="flex space-x-2">
                            <Link href={route('admin.membro_quadros.trash')}>
                                <Button variant="outline">
                                    <Trash2 className="mr-2 h-4 w-4" />
                                    Lixo
                                </Button>
                            </Link>
                            <Link href={route('admin.membro_quadros.create')}>
                                <Button>
                                    <Plus className="mr-2 h-4 w-4" />
                                    Criar Membro de Quadro
                                </Button>
                            </Link>
                        </div>
                    </div>
                ...
                <CardContent>
                    <form onSubmit={handleSearch} className="mb-4 flex space-x-2">
                        <div className="relative flex-1">
                            ...
                            <Input
                                type="text"
                                placeholder="Pesquisar membro de quadro..."
                                value={search}
                                ...
                                {items.data.map((item) => (
                                    <tr key={item.id}>
                                        ...
                                        <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                                            <div className="flex space-x-2">
                                                <Link href={route('admin.membro_quadros.show', item.id)}>
                                                    <Button variant="outline" size="sm">Ver</Button>
                                                </Link>
                                                <Link href={route('admin.membro_quadros.edit', item.id)}>
