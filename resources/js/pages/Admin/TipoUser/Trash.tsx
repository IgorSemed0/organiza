...
export default function Trash({ items, filters }) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        router.get(route('admin.tipo_users.trash'), { search }, { preserveState: true });
    };

    const handleRestore = (id: number) => {
        if (confirm('Tem a certeza que deseja restaurar este tipo de utilizador?')) {
            post(route('admin.tipo_users.restore', id), {
                onSuccess: () => {
                    reset();
    ...
    const handlePurge = (id: number) => {
        if (confirm('Tem a certeza que deseja eliminar permanentemente este tipo de utilizador?')) {
            deleteRequest(route('admin.tipo_users.purge', id), {
                onSuccess: () => {
                    reset();
                    router.reload({ only: ['items', 'filters'] });
                },
    ...
    return (
        <AdminLayout title="Lixo de Tipos de Utilizador">
            <Head title="Lixo de Tipos de Utilizador" />
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <CardTitle>Lixo de Tipos de Utilizador</CardTitle>
                ...
                <CardContent>
                    ...
                    <form onSubmit={handleSearch} className="mb-4 flex space-x-2">
                        <div className="relative flex-1">
                            ...
                            <Input
                                type="text"
                                placeholder="Pesquisar tipo de utilizador eliminado..."
                                value={search}
