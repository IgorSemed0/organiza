...
export default function Index({ items, filters }) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        router.get(route('admin.membro_quadro_convites.index'), { search }, { preserveState: true });
    };

    const handleDelete = (id: number) => {
        if (confirm('Tem a certeza que deseja eliminar este convite para quadro?')) {
            router.delete(route('admin.membro_quadro_convites.destroy', id));
        }
    };

    return (
        <AdminLayout title="Gestão de Convites para Quadros">
            <Head title="Gestão de Convites para Quadros" />
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <CardTitle>Gestão de Convites para Quadros</CardTitle>
                        <div className="flex space-x-2">
                            <Link href={route('admin.membro_quadro_convites.trash')}>
                                <Button variant="outline">
                                    <Trash2 className="mr-2 h-4 w-4" />
                                    Lixo
                                </Button>
                            </Link>
                            <Link href={route('admin.membro_quadro_convites.create')}>
                                <Button>
                                    <Plus className="mr-2 h-4 w-4" />
                                    Criar Convite para Quadro
                                </Button>
                            </Link>
                        </div>
                    </div>
                ...
                <CardContent>
                    <form onSubmit={handleSearch} className="mb-4 flex space-x-2">
                        <div className="relative flex-1">
                            ...
                            <Input
                                type="text"
                                placeholder="Pesquisar convite para quadro..."
                                value={search}
                                ...
                                {items.data.map((item) => (
                                    <tr key={item.id}>
                                        ...
                                        <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                                            <div className="flex space-x-2">
                                                <Link href={route('admin.membro_quadro_convites.show', item.id)}>
                                                    <Button variant="outline" size="sm">Ver</Button>
                                                </Link>
                                                <Link href={route('admin.membro_quadro_convites.edit', item.id)}>
