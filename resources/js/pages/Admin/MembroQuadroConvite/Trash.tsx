...
export default function Trash({ items, filters }) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        router.get(route('admin.membro_quadro_convites.trash'), { search }, { preserveState: true });
    };

    const handleRestore = (id: number) => {
        if (confirm('Tem a certeza que deseja restaurar este convite para quadro?')) {
            post(route('admin.membro_quadro_convites.restore', id), {
                onSuccess: () => {
                    reset();
    ...
    const handlePurge = (id: number) => {
        if (confirm('Tem a certeza que deseja eliminar permanentemente este convite para quadro?')) {
            deleteRequest(route('admin.membro_quadro_convites.purge', id), {
                onSuccess: () => {
                    reset();
                    router.reload({ only: ['items', 'filters'] });
                },
    ...
    return (
        <AdminLayout title="Lixo de Convites para Quadros">
            <Head title="Lixo de Convites para Quadros" />
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <CardTitle>Lixo de Convites para Quadros</CardTitle>
                ...
                <CardContent>
                    ...
                    <form onSubmit={handleSearch} className="mb-4 flex space-x-2">
                        <div className="relative flex-1">
                            ...
                            <Input
                                type="text"
                                placeholder="Pesquisar convite para quadro eliminado..."
                                value={search}
