...
export default function Index({ items, filters }) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        router.get(route('admin.listas_verificacaos.index'), { search }, { preserveState: true });
    };

    const handleDelete = (id: number) => {
        if (confirm('Tem a certeza que deseja eliminar esta lista de verificação?')) {
            router.delete(route('admin.listas_verificacaos.destroy', id));
        }
    };

    return (
        <AdminLayout title="Gestão de Listas de Verificação">
            <Head title="Gestão de Listas de Verificação" />
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <CardTitle>Gestão de Listas de Verificação</CardTitle>
                        <div className="flex space-x-2">
                            <Link href={route('admin.listas_verificacaos.trash')}>
                                <Button variant="outline">
                                    <Trash2 className="mr-2 h-4 w-4" />
                                    Lixo
                                </Button>
                            </Link>
                            <Link href={route('admin.listas_verificacaos.create')}>
                                <Button>
                ...
                <CardContent>
                    <form onSubmit={handleSearch} className="mb-4 flex space-x-2">
                        <div className="relative flex-1">
                            ...
                            <Input
                                type="text"
                                placeholder="Pesquisar lista de verificação..."
                                value={search}
                                ...
                                {items.data.map((item) => (
                                    <tr key={item.id}>
                                        ...
                                        <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                                            <div className="flex space-x-2">
                                                <Link href={route('admin.listas_verificacaos.show', item.id)}>
                                                    <Button variant="outline" size="sm">Ver</Button>
                                                </Link>
                                                <Link href={route('admin.listas_verificacaos.edit', item.id)}>
